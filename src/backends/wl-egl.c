...
void wlclient_egl_add_config_attr(EGLint key, EGLint value) {
    WLCLIENT_ASSERT((g_config_attribs_count + 2) < WLCLIENT_EGL_MAX_ATTRIBUTES);
...
void wlclient_egl_add_context_attr(EGLint key, EGLint value) {
    WLCLIENT_ASSERT((g_context_attribs_count + 2) < WLCLIENT_EGL_MAX_ATTRIBUTES);
...
wlclient_error_code wlclient_egl_init(EGLenum api) {
    WLCLIENT_LOG_DEBUG("Initializing EGL backend...");

    i32 config_count;
    EGLint major = 0;
    EGLint minor = 0;

    g_egl_display = eglGetDisplay((EGLNativeDisplayType) wlclient_get_wl_display());
    ...
    WLCLIENT_LOG_DEBUG("EGL version %" PRIi32 ".%" PRIi32, major, minor);
    ...
    wlclient_set_backend_shutdown(egl_shutdown);
    wlclient_set_backend_destroy_window(egl_destroy_window);
    wlclient_set_backend_resize_window(egl_resize_window);
